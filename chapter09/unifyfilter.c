...
AVFilterContext *buffersrc_ctx = NULL; // 视频输入滤镜的实例
AVFilterContext *buffersink_ctx = NULL; // 视频输出滤镜的实例
AVFilterGraph *filter_graph = NULL; // 视频滤镜图
AVFilterContext *abuffersrc_ctx = NULL; // 音频输入滤镜的实例
AVFilterContext *abuffersink_ctx = NULL; // 音频输出滤镜的实例
AVFilterGraph *afilter_graph = NULL; // 音频滤镜图

// 提取指定滤镜的描述字符串
char *get_filter_desc(const char *filters_desc, const char *filter_prefix) {
    char *ptr = NULL;
    ...
    while (value) {
        if (strstr(value, filter_prefix) != NULL) {
    ...
    av_log(NULL, AV_LOG_INFO, "find filter desc: %s\n", ptr+strlen(filter_prefix));
...
// 初始化视频滤镜（也称过滤器、滤波器）
int init_video_filter(const char *filters_desc) {
    av_log(NULL, AV_LOG_INFO, "video filters_desc = %s\n", filters_desc);
    int ret = 0;
    ...
    av_log(NULL, AV_LOG_INFO, "Success initialize video filter.\n");
    return ret;
}

// 初始化音频滤镜（也称过滤器、滤波器）
int init_audio_filter(const char *filters_desc) {
    av_log(NULL, AV_LOG_INFO, "audio filters_desc = %s\n", filters_desc);
    int ret = 0;
    ...
    avfilter_inout_free(&inputs); // 释放滤镜的输入参数
    avfilter_inout_free(&outputs); // 释放滤镜的输出参数
    av_log(NULL, AV_LOG_INFO, "Success initialize audio filter.\n");
    return ret;
}
...
int main(int argc, char **argv) {
    ...
    const char *dest_name = "output_unify.mp4";
    ...
    init_video_filter(video_filters_desc); // 初始化视频滤镜
    init_audio_filter(audio_filters_desc); // 初始化音频滤镜
    ...
    while (av_read_frame(in_fmt_ctx, packet) >= 0) { // 轮询数据包
        ...
        } else if (packet->stream_index == audio_index) { // 音频包需要重新编码
            packet->stream_index = 1;
    ...
    avfilter_free(buffersrc_ctx); // 释放视频输入滤镜的实例
    avfilter_free(buffersink_ctx); // 释放视频输出滤镜的实例
    avfilter_graph_free(&filter_graph); // 释放视频滤镜图资源
    avfilter_free(abuffersrc_ctx); // 释放音频输入滤镜的实例
    avfilter_free(abuffersink_ctx); // 释放音频输出滤镜的实例
    avfilter_graph_free(&afilter_graph); // 释放音频滤镜图资源
