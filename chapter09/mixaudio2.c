...
int main(int argc, char **argv) {
    ...
    const char *dest_name = "output_mixaudio2.mp3";
    ...
    int64_t last_pts=0, increment_pts=0; // 最近的时间戳，时间戳增量
    int i = 0;
    int ret = -1;
    AVPacket *packet[2];
    ...
    while (av_read_frame(in_fmt_ctx[0], packet[0]) >= 0) { // 轮询数据包
        if (packet[0]->stream_index == audio_index[0]) { // 音频包需要重新编码
            if (i == 1) {
                increment_pts = packet[0]->pts - last_pts;
            }
            i++;
            last_pts = packet[0]->pts; // 保存最新的时间戳
            ret = recode_audio(packet, frame, filt_frame); // 对音频帧重新编码
        }
        av_packet_unref(packet[0]); // 清除数据包
    }
    av_log(NULL, AV_LOG_INFO, "increment_pts=%lld\n", increment_pts);
    while (av_read_frame(in_fmt_ctx[1], packet[1]) >= 0) { // 轮询数据包
        if (packet[1]->stream_index == audio_index[1]) { // 音频包需要重新编码
            // 把未解压的数据包发给解码器实例
            int ret = avcodec_send_packet(audio_decode_ctx[1], packet[1]);
            if (ret == 0) {
                // 从解码器实例获取还原后的数据帧
                ret = avcodec_receive_frame(audio_decode_ctx[1], frame[1]);
                if (ret == AVERROR(EAGAIN) || ret == AVERROR_EOF) {
                    break;
                } else if (ret < 0) {
                    av_log(NULL, AV_LOG_ERROR, "decode frame occur error %d.\n", ret);
                ...
                if (ret == 0) {
                    while (1) {
                        ...
                        last_pts += increment_pts;
                        filt_frame->pts = last_pts; // 使用第一个文件的时间戳
                        output_audio(filt_frame); // 给音频帧编码，并写入压缩后的音频包
                    }
                } else {
    ...
    av_log(NULL, AV_LOG_INFO, "Success mix audio file.\n");
