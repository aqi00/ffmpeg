...
static const AVOption xfade_options[] = {
    ...
    {   "wipelefttop",   "wipe left top transition",   0, AV_OPT_TYPE_CONST, {.i64=WIPELEFTTOP},   0, 0, FLAGS, "transition" },
    {   "turnpage",      "turn page transition",       0, AV_OPT_TYPE_CONST, {.i64=TURNPAGE},      0, 0, FLAGS, "transition" },
...
#define WIPELEFTTOP_TRANSITION(name, type, div)                                      \
static void wipelefttop##name##_transition(AVFilterContext *ctx,                     \
                                const AVFrame *a, const AVFrame *b, AVFrame *out,    \
                                float progress,                                      \
                                int slice_start, int slice_end, int jobnr)           \
{                                                                                    \
    ...
    const int zw = out->width * progress;                                            \
    const int zh = out->height * progress;                                           \
    const float slope = -1.0 * out->height / out->width;                             \
    const float xieb = zh - slope*zw;                                                \
                                                                                     \
    for (int p = 0; p < s->nb_planes; p++) {                                         \
        const type *xf0 = (const type *)(a->data[p] + slice_start * a->linesize[p]); \
        const type *xf1 = (const type *)(b->data[p] + slice_start * b->linesize[p]); \
        ...
        for (int y = 0; y < height; y++) {                                           \
            for (int x = 0; x < out->width; x++) {                                   \
                dst[x] = slice_start + y  > x*slope+xieb ? xf1[x] : xf0[x];          \
            }                                                                        \
                                                                                     \
            dst += out->linesize[p] / div;                                           \
            xf0 += a->linesize[p] / div;                                             \
...
WIPELEFTTOP_TRANSITION(8, uint8_t, 1)
WIPELEFTTOP_TRANSITION(16, uint16_t, 2)

#define TURNPAGE_TRANSITION(name, type, div)                                         \
static void turnpage##name##_transition(AVFilterContext *ctx,                        \
                                const AVFrame *a, const AVFrame *b, AVFrame *out,    \
                                float progress,                                      \
                                int slice_start, int slice_end, int jobnr)           \
{                                                                                    \
    ...
    const int zw = out->width * progress;                                            \
    const int zh = out->height * progress;                                           \
    calcSlope(out->width, out->height, zw, out->height/2+zh/2);                      \
                                                                                     \
    for (int p = 0; p < s->nb_planes; p++) {                                         \
        const int bg = s->white[p];                                                  \
        const type *xf0 = (const type *)(a->data[p] + slice_start * a->linesize[p]); \
        ...
        for (int y = 0; y < height; y++) {                                           \
            for (int x = 0; x < out->width; x++) {                                   \
                int show_type = calcShowType(x, slice_start+y);                      \
                if (show_type == 1) {                                                \
                    dst[x] = bg;                                                     \
                } else if (show_type == 2) {                                         \
                    dst[x] = xf1[x];                                                 \
...
TURNPAGE_TRANSITION(8, uint8_t, 1)
TURNPAGE_TRANSITION(16, uint16_t, 2)
...
static int config_output(AVFilterLink *outlink)
{
    ...
    case WIPELEFTTOP:   s->transitionf = s->depth <= 8 ? wipelefttop8_transition   : wipelefttop16_transition;   break;
    case TURNPAGE:   s->transitionf = s->depth <= 8 ? turnpage8_transition   : turnpage16_transition;   break;
