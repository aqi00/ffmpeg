...
int save_png_file(AVFrame *frame, int save_index) {
    ...
    AVFrame *rgb_frame = av_frame_alloc(); // 分配一个RGB数据帧
...
//                       target_format, frame->width, frame->height, 1);
    ...
    if (!png_encode_ctx) {
        av_log(NULL, AV_LOG_ERROR, "png_encode_ctx is null\n");
        return -1;
    }
    ...
    if (ret < 0) {
        av_log(NULL, AV_LOG_ERROR, "Can't open png_encode_ctx.\n");
        return -1;
    }
...
int decode_video(AVPacket *packet, AVFrame *frame, int save_index) {
    ...
    while (1) {
        ...
        save_png_file(frame, save_index); // 把视频帧保存为PNG图片
        break;
    }
    return ret;
}
