...
import com.example.exoplayer.util.PermissionUtil;
...
public class MainActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        findViewById(R.id.btn_video_edit).setOnClickListener(this);
...
//            }
//        } else
            if (v.getId() == R.id.btn_exo_player) {
                if (PermissionUtil.checkPermission(this, new String[]{Manifest.permission.WRITE_EXTERNAL_STORAGE}, (int) v.getId() % 65536)) {
                    startActivity(new Intent(this, ExoPlayerActivity.class));
                }
            } else if (v.getId() == R.id.btn_video_edit) {
                if (PermissionUtil.checkPermission(this, new String[]{Manifest.permission.WRITE_EXTERNAL_STORAGE}, (int) v.getId() % 65536)) {
...
//            }
//        } else
            if (requestCode == R.id.btn_exo_player % 65536) {
                if (PermissionUtil.checkGrant(grantResults)) { // 用户选择了同意授权
                    startActivity(new Intent(this, ExoPlayerActivity.class));
                } else {
                    Toast.makeText(this, "需要允许存储权限才能播放视频噢", Toast.LENGTH_SHORT).show();
                }
            } else if (requestCode == R.id.btn_video_edit % 65536) {
                    if (PermissionUtil.checkGrant(grantResults)) { // 用户选择了同意授权
                        startActivity(new Intent(this, VideoEditActivity.class));
                    } else {
                        Toast.makeText(this, "需要允许存储权限才能编辑视频噢", Toast.LENGTH_SHORT).show();
                    }
