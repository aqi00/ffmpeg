...
SwrContext *swr_ctx = NULL; // 采样器的实例
...
int thread_work(void *arg) {
    while (1) {
        ...
        AVPacket packet = packet_list.front(); // 取出头部的视频包
...
int prepare_video(void) {
    ...
    interval = 1000 / fps; // 根据帧率计算每帧之间的播放间隔
    // 初始化SDL
...
int prepare_audio(void) {
    ...
    int ret = swr_alloc_set_opts2(&swr_ctx, // 采样器的实例
...
int main(int argc, char **argv) {
    ...
    while (av_read_frame(in_fmt_ctx, packet) >= 0) { // 轮询数据包
        if (packet->stream_index == audio_index) { // 音频包需要解码
            ...
            SDL_LockMutex(list_lock); // 对队列锁加锁
            packet_list.push_back(*packet); // 把视频包加入队列
            SDL_UnlockMutex(list_lock); // 对队列锁解锁
            if (!has_audio) { // 不存在音频流
