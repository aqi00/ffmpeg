...
int open_output_file(const char *dest_name) {
    ...
    if (video_index >= 0) { // 创建编码器实例和新的视频流
        ...
        av_log(NULL, AV_LOG_INFO, "pix_names: %s\n", pix_names);
    }
...
int main(int argc, char **argv) {
    ...
    av_log(NULL, AV_LOG_INFO, "colorspace: %s\n", colorspace);
    if (open_input_file(src_name, colorspace) < 0) { // 打开输入文件
        return -1;
    }
    ...
    av_log(NULL, AV_LOG_INFO, "Success process png file.\n");
